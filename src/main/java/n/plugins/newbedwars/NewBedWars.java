...
import n.plugins.newbedwars.manager.EntityCleanupManager;
...
public final class NewBedWars extends JavaPlugin {
    ...
    private EntityCleanupManager entityCleanupManager;
    ...
    public void onEnable() {
        ...
        this.entityCleanupManager = new EntityCleanupManager(this);
    ...
    public void onDisable() {
        ...
        if (entityCleanupManager != null) {
            entityCleanupManager.shutdown();
        }
    ...
    public boolean reloadPluginConfiguration() {
        if (setupManager != null && setupManager.hasActiveSessions()) {
            return false;
        }
        if (arenaManager != null && !arenaManager.getRuntimeArenas().isEmpty()) {
            return false;
        }
        ...
        gameManager.invalidateConfigurationCache();
        generatorManager.invalidateConfigurationCache();
        scoreboardManager.invalidateConfigurationCache();
        npcManager.invalidateConfigurationCache();
        scoreboardManager.restartScheduler();
        npcManager.restartScheduler();
        arenaManager.loadArenas();
        lobbyManager.applyConfiguredLobbyWorldRules();
        return true;
    }
    ...
    private void updateConfigurationIfNeeded() {
        ...
        try {
            ...
            int installedVersion = getConfig().getInt("config-version", 0);
            ...
            if (installedVersion < 2 && getConfig().getLong("scoreboard.update-ticks", 10L) == 10L) {
                getConfig().set("scoreboard.update-ticks", 20L);
            }
            if (installedVersion < 4) {
                if ("&b&lBEDWARS".equals(getConfig().getString("scoreboard.title"))) {
                    getConfig().set("scoreboard.title", "&b&lNEW&f&lBEDWARS");
                }
                migrateBrandList("scoreboard.lobby");
                migrateBrandList("scoreboard.waiting");
                migrateBrandList("scoreboard.starting");
                migrateBrandList("scoreboard.ingame");
                migrateBrandList("scoreboard.ending");
                migrateBrandList("tablist.header");
            }
            if (installedVersion < 5
                && "&b&lLOJA DE ITENS".equals(getConfig().getString("npc.item-shop.name"))) {
                getConfig().set("npc.item-shop.name", "&b&lLOJA");
            }
            getConfig().set("config-version", CURRENT_CONFIG_VERSION);
            saveConfig();
            getLogger().info("config.yml atualizado para a estrutura " + CURRENT_CONFIG_VERSION + ".");
        } finally {
            try {
                resource.close();
    ...
    private void migrateBrandList(String path) {
        java.util.List<String> lines = getConfig().getStringList(path);
        if (lines == null || lines.isEmpty()) {
            return;
        }

        boolean changed = false;
        java.util.List<String> migrated = new java.util.ArrayList<String>(lines.size());
        for (String line : lines) {
            String replacement = line;
            if ("&b&lBEDWARS".equals(line)) {
                replacement = "&b&lNEW&f&lBEDWARS";
            } else if ("&fnewplugins.com".equals(line)
                || "&enewplugins.net".equals(line)) {
                replacement = "&bNewBedWars";
            }
            migrated.add(replacement);
            changed |= replacement == null ? line != null : !replacement.equals(line);
        }
        if (changed) {
    ...
    private void registerListeners() {
        ...
        Bukkit.getPluginManager().registerEvents(entityCleanupManager, this);
    }
