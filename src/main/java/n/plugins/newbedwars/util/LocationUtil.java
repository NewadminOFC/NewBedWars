...
public final class LocationUtil {
    ...
    public static Location loadLocation(ConfigurationSection section, String path) {
        return loadLocation(section, path, null, true);
    }

    /**
     * Loads coordinates without forcing an arena template world to start.
     *
     * <p>Bukkit's {@link Location} can safely keep a {@code null} world until the
     * arena is actually used. Arena code relocates these coordinates to the
     * active match world. This prevents every configured BedWars map from being
     * loaded and ticking permanently during plugin startup.</p>
     */
    public static Location loadLocationLazy(ConfigurationSection section, String path, String fallbackWorldName) {
        return loadLocation(section, path, fallbackWorldName, false);
    }

    private static Location loadLocation(ConfigurationSection section, String path, String fallbackWorldName, boolean loadWorld) {
        if (!section.isConfigurationSection(path)) {
            return null;
        }

        String worldName = section.getString(path + ".world", fallbackWorldName);
        if (worldName == null || worldName.trim().isEmpty()) {
            return null;
        }
        World world = loadWorld ? ensureWorldLoaded(worldName) : Bukkit.getWorld(worldName);
    ...
    public static void saveLocation(ConfigurationSection section, String path, Location location, String fallbackWorldName) {
        if (location == null) {
            section.set(path, null);
            return;
        }
        ...
        String worldName = world == null ? fallbackWorldName : world.getName();
        if (worldName == null || worldName.trim().isEmpty()) {
            section.set(path, null);
            return;
        }
    ...
    public static CuboidRegion relocate(CuboidRegion region, World world) {
        ...
        return pos1 == null || pos2 == null ? null : new CuboidRegion(pos1, pos2, world.getName());
    }
