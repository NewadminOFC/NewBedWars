...
public class TeamManager {
    ...
    public List<UUID> getPlayersInTeam(Arena arena, TeamColor color) {
        List<UUID> players = new ArrayList<UUID>(4);
    ...
    public boolean isTeamAvailable(Arena arena, UUID uniqueId, TeamColor color) {
        ...
        return containsPlayer || count < getTeamSize(arena);
    }
    ...
    private final Map<String, Long> eliminationCacheTime = new HashMap<String, Long>();
    private final Map<String, List<ArenaTeam>> aliveTeamsCache = new HashMap<String, List<ArenaTeam>>();

    public List<ArenaTeam> getAliveTeams(Arena arena) {
        if (arena == null) {
            return new ArrayList<ArenaTeam>(0);
        }

        String key = arena.getName();
        long now = System.currentTimeMillis();
        Long cached = eliminationCacheTime.get(key);
        if (cached != null && now - cached.longValue() < 900L) {
            List<ArenaTeam> result = aliveTeamsCache.get(key);
            if (result != null) {
                return result;
            }
        }
        ...
        updateEliminationState(arena);
        List<TeamColor> colors = getActiveColors(arena);
        List<ArenaTeam> alive = new ArrayList<ArenaTeam>(colors.size());
        ...
        eliminationCacheTime.put(key, Long.valueOf(now));
        aliveTeamsCache.put(key, alive);
        return alive;
    }

    public void invalidateAliveTeamsCache(Arena arena) {
        if (arena == null) {
            return;
        }
        String key = arena.getName();
        eliminationCacheTime.remove(key);
        aliveTeamsCache.remove(key);
    }
